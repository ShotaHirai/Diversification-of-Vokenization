...
class Vokenizer:
    ...
    def vokenize_sents(self, sents, topk=None, sampling=False, random=False, topP=False, topP_value=0.0):
        ...
        return self.vokenize_ids(input_ids, attention_mask=None, topk=topk, sampling=sampling, random=random, topP=topP, topP_value=topP_value)

    def vokenize_ids(self, input_ids, attention_mask=None, topk=None, sampling=False, random=False, topP=False, topP_value=0.0):
        """
        ...
        def apply_model(input_ids, attention_mask, lengths):
            ...
            # Visn retrieval
            if random:
                top_score, top_idx = self.indexer.random(gathered_output)
            else:
                if sampling:
                    top_score, top_idx = self.indexer.batch_sampling_from_topk(gathered_output, topk)
                elif topP:
                    top_score, top_idx = self.indexer.batch_topP_sampling(gathered_output, topk=2048, topP_value=topP_value)
                else:
                    if topk is None:
