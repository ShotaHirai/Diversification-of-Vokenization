...
class FaissGPUIndexer(GPUIndexer):
    ...
    def batch_sampling_from_topk(self, query, topk: int = 1):
        if type(query) is torch.Tensor:
            query = query.cpu().numpy()
        D, I = self.gpu_index_flat.search(query, topk)
        inverse_D = np.reciprocal(D)
        score = []
        ids = []
        for index, inverse_d in enumerate(inverse_D):
            pk = inverse_d/inverse_d.sum()
            xk = np.arange(topk)
            custm = stats.rv_discrete(name='custm', values=(xk, pk))
            sampled_rvs = custm.rvs(size=1)
            score.append(D[index][sampled_rvs][0])
            ids.append(I[index][sampled_rvs][0])
        D = torch.tensor(score)
        I = torch.tensor(ids)
        return D, I

    def batch_topP_sampling(self, query, topk: int = 100, topP_value: float = 0.9):
        if type(query) is torch.Tensor:
            query = query.cpu().numpy()
        D, I = self.gpu_index_flat.search(query, topk)
        inverse_D = np.reciprocal(D)
        ...
        for index, inverse_d in enumerate(inverse_D):
            pk = inverse_d/inverse_d.sum()
            threshold = np.argmax(np.cumsum(pk) > topP_value)
            xk = np.arange(threshold)
            custm = stats.rv_discrete(name='custm', values=(xk, pk[:threshold]/pk[:threshold].sum()))
            sampled_rvs = custm.rvs(size=1)
            score.append(D[index][sampled_rvs][0])
            ids.append(I[index][sampled_rvs][0])
        D = torch.tensor(score)
    ...
    # queryに対してランダムなkey:Iを返す
    def random(self, query):
        if type(query) is torch.Tensor:
            query = query.cpu().numpy()
        I = np.random.randint(0, self.keys.shape[0]-1, (len(query), 1))
        D = np.zeros((len(query), 1))
        D = torch.from_numpy(D)
        I = torch.from_numpy(I)
        return D, I
