...
def processer(args, input_queue, output_queue):
    ...
    while True:
        ...
        if sents is not None:
            output_str = ''
            results = vokenizer.vokenize_ids(sents, topk=args.topk, sampling = args.sampling, random=args.random, topP=args.topP, topP_value=args.topP_value)
            idxs = results[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sampling', action="store_true",
                        help='sampling')
    parser.add_argument('--topP', action="store_true",
                        help='top-P sampling')
    parser.add_argument('--topP_value', type=float, default=0.9,
                        help='top-P value')
    parser.add_argument('--random', action="store_true",
                        help='if vokenization apply random vokens')
    parser.add_argument('--topk', type=int, default=1,
                        help='How many top-vokens are sampled')
